...
directive_table <- data.frame(
  directive_id = "D1",
  source = "feedback_intake.md",
  directive = "Replace this placeholder with a direct user-feedback item.",
  status = "addressed",
...
cards <- list(
  list(
    ...
    feedback = c(
      "Direct feedback addressed: summarize the concrete change visible in this figure."
    ),
    caveats = c(
...
appendix_html <- function(paths) {
  ...
  paste0(
    "<details class=\"appendix-block\"><summary><strong>Appendix: full notes and feedback files</strong></summary>",
...
html <- paste0(
  ...
  "<p>Each figure is paired with its legend, feedback responses, caveats, and local source files. Recommended final drafts appear before alternatives; raw refined and initial galleries should be collapsed unless they are the primary review item.</p></div>\n",
