#!/usr/bin/env python3
"""Validate manuscript polishing-gate input and output contracts."""
...
def check_figure_script_content(reporter: Reporter, path: Path, contract: dict[str, Any], label: str) -> None:
    try:
    ...
    if not contract.get("allow_raster_assembly", False) and RASTER_ASSEMBLY_RE.search(script_text):
        reporter.error(
            f"{label} appears to assemble figures from raster image files. "
            "Final composites should be built from regenerated plot/grob objects unless allow_raster_assembly is true."
        )
...
def parse_positive_float(value: str, label: str, reporter: Reporter) -> float | None:
    try:
        parsed = float(value)
...
def check_feedback(reporter: Reporter, path: Path, contract: dict[str, Any]) -> None:
    if not check_file_exists(reporter, path, "feedback file"):
        return
    text = path.read_text(encoding="utf-8", errors="replace").strip()
    if not text:
        reporter.error(f"Feedback file is blank: {path}")
        return
    markers = as_list(contract.get("approval_markers")) or [
        "APPROVED",
        "approved",
    ...
    if contract.get("approval_required", False) and not has_marker:
        reporter.error(f"Feedback file lacks required approval marker: {path}")
    elif not has_marker:
        reporter.warn(f"Feedback file has no explicit approval marker: {path}")
    else:
        reporter.pass_(f"Feedback approval marker detected: {path}")
...
def run_validation(contract: dict[str, Any], phase: str) -> Reporter:
    ...
    feedback = resolve(root, contract.get("feedback_file"))
    if feedback is None:
        reporter.error("Contract missing required field: feedback_file")
    else:
        check_feedback(reporter, feedback, contract)
    ...
    default_optimizer = ".agents/skills/manuscript-polishing-gate/scripts/optimize_panel_layout.R"
    ...
    if phase in {"postflight", "both"}:
        ...
        if require_regenerated:
            ...
            if figure_script is not None:
                ...
                check_figure_script_content(reporter, figure_script, contract, "Figure polishing script")
            ...
            if allow_split_scripts:
                ...
                if assembly_script is not None:
                    ...
                    check_figure_script_content(reporter, assembly_script, contract, "Figure assembly script")
        ...
        if contract.get("require_legend_files", True) and not legend_files:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("contract", type=Path, help="Polishing gate contract JSON")
    parser.add_argument(
        "--phase",
