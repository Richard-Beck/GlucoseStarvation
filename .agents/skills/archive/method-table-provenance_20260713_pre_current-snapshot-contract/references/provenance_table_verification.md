...
that the declared manuscript-visible figure set is exactly the set locked by
...
`target_figure_set.tsv`, then run:
...
text. The verifier owns and enforces the required target-manifest and locked
provenance endpoint-table shapes. Do not infer endpoint mappings or adapt
nonconforming tables manually.
