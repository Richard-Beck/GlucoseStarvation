...
accepted as an exception. Choose targeted update only when unaffected rows and
branches can be authenticated from the verifier results; otherwise use clean
reconstruction. Repair or reconstruct the affected provenance table, then rerun
the verifier before Methods validation.
...
Do not fully retrace authenticated unaffected branches by default. Reconstruct
provenance for each endpoint that fails deterministic validation, including any
shared ancestor whose current identity cannot be authenticated.
