functions {
  ...
  vector model_b_ode(real t, vector y, array[] real p) {
    ...
    // --- Absolute Glucose Logic ---
    real k_smooth = 100.0;
    // y[3] is absolute glucose (mM). Apply smoothing directly.
    ...
    // Derivative is not divided by G0
  ...
  real partial_sum_lpmf(
      ...
      int start, int end,
      array[] int line_id,
      vector ploidy_metric,              // CHANGED: was array[] int high_p
      array[] int exp_id, vector G0_per_well, array[] real t_grid,
      ...
      array[] int is_censored,            // CHANGED: added censoring flags for glucose obs
    ...
    for (i in 1:size(slice_wells)) {
      int w = slice_wells[i];
      int l = line_id[w];
      real h = ploidy_metric[w];         // CHANGED: continuous covariate

      array[10] real p_w;
      for (pp in 1:10) {
        real raw = mu_global[pp] + sigma_line[pp] * z_line[pp, l] + beta_high[pp] * h; // CHANGED: * h (real)
        if (pp == 6 || pp == 8) {
      ...
      vector[3] y0_w;
      y0_w[1] = mu_IC[1] + sigma_IC[1] * z_IC[1, l] + beta_IC[1] * h; // CHANGED: * h (real)
      y0_w[2] = mu_IC[2] + sigma_IC[2] * z_IC[2, l] + beta_IC[2] * h; // CHANGED: * h (real)
      y0_w[3] = G0;
...
data {
  ...
  vector[N_wells] ploidy_metric;                 // CHANGED: was array[N_wells] int is_high_ploidy;
  ...
  array[N_obs_gluc] int<lower=0,upper=1> is_censored;   // CHANGED: added
...
model {
  ...
  if (mode == 0) {
    ...
    target += reduce_sum(
      ...
      seq_wells,
      grainsize,
      line_id, ploidy_metric, exp_id, G0_per_well, t_grid,                    // CHANGED
...
generated quantities {
  ...
  if (mode != 1 && calc_sim == 1) {
    ...
    for (w in 1:N_wells) {
      int l = line_id[w];
      real h = ploidy_metric[w];  // CHANGED
      array[10] real p_w;

      for (pp in 1:10) {
        real raw = mu_global[pp] + sigma_line[pp] * z_line[pp, l] + beta_high[pp] * h; // CHANGED
      ...
      vector[3] y0_w;
      y0_w[1] = mu_IC[1] + sigma_IC[1] * z_IC[1, l] + beta_IC[1] * h; // CHANGED
      y0_w[2] = mu_IC[2] + sigma_IC[2] * z_IC[2, l] + beta_IC[2] * h; // CHANGED
      y0_w[3] = G0;
  ...
  if (mode == 0) {
    if (calc_sim == 1) {
       ...
       for (n in 1:N_obs_gluc) {
          ...
          // CHANGED: censoring support in log_lik too
          if (is_censored[n]) {
            log_lik += lognormal_lcdf(lum_obs[n] | log(mu + 1e-12), calib_sigma_fixed[e]);
          } else {
            log_lik += lognormal_lpdf(lum_obs[n] | log(mu + 1e-12), calib_sigma_fixed[e]);
