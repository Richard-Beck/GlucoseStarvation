...
NUM_THREADS <- if (length(args) >= 4) as.integer(args[4]) else 4
NUM_PATHS   <- if (length(args) >= 5) as.integer(args[5]) else 60
...
save_path <- file.path(outDir, paste0("pathfinder_", MODEL_NAME, ".Rds"))
fit_pf$save_object(file = save_path)

cat(sprintf("\nPathfinder complete. Saved to %s\n", save_path))
print(fit_pf$summary(variables = c("logp", "mu_IC", "phi_total", "phi_frac")))
