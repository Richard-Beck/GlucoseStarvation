...
# ---- NUTS controls ----
CHAIN_ID      <- if (length(args) >= 8)  as.integer(args[8])  else 1L
ITER_WARMUP   <- if (length(args) >= 9)  as.integer(args[9])  else 1000L
ITER_SAMPLING <- if (length(args) >= 10) as.integer(args[10]) else 1000L
ADAPT_DELTA   <- if (length(args) >= 11) as.numeric(args[11]) else 0.9
MAX_TREED     <- if (length(args) >= 12) as.integer(args[12]) else 12L
HOLDOUT <- FALSE
...
run_id <- sprintf("%dR_%dP_%dW_C%d_M%d",
                  R_val, P_val, W_val, CONSTRAINT_FLAG, WASTE_MECH_FLAG)

source("ecology/model_selection/models/gpath/gpath.R")

cat(sprintf(
  ">>> RUNNING NUTS | %s | %s | chain=%d | warmup=%d | sampling=%d\n",
  MODEL_NAME, run_id, CHAIN_ID, ITER_WARMUP, ITER_SAMPLING
))
...
if (W_val > 0) {
  stan_data$waste_mech <- rep(as.numeric(WASTE_MECH_FLAG), W_val)
} else {
  stan_data$waste_mech <- numeric(0)
}
...
nutsDir <- file.path(outDir, "nuts")
dir.create(nutsDir, recursive = TRUE, showWarnings = FALSE)

outfile <- file.path(nutsDir, paste0(CHAIN_ID, ".Rds"))
cat(sprintf(">>> Saving draws to %s\n", outfile))

# Optional: skip completed chains
...
# Names in the Stan parameters block (keep this in sync with your stan file)
PARAM_NAMES <- c(
  "raw_theta_line",
  "raw_theta_ploidy",
  "raw_N0",
  ...
  "sigma_base",
  "sigma_rel",
  "G1_0"          # <-- Changed from "G1_0_raw"
)

# Clamp for constrained parameters
clamp_constrained <- function(init) {
  ...
  if (!is.null(init$G1_0))       init$G1_0       <- pmax(init$G1_0,      1e-12) # <-- Added clamp for G1_0 lower bound
  init
}

flat_to_init_list <- function(x_named, param_names = PARAM_NAMES) {
  # Drop internal Stan diagnostics / lp__ etc.
  ...
  # Scalars: name without '['
  scal <- !grepl("\\[", nms)
  if (any(scal)) {
    for (nm in nms[scal]) {
      if (nm %in% param_names) out[[nm]] <- unname(x_named[[nm]])
    }
  }
  ...
  if (length(idx_nms)) {
    ...
    for (bn in unique(base)) {
      ...
      # FIXED: Changed sub() to gsub() to strip both brackets
...
# ---- 5c. Choose init from ranked optimization fits using CHAIN_ID ----
init_arg <- 2  # fallback
#opt_rc_path <- file.path(outDir, "optim_rc_all.Rds")
...
if (file.exists(opt_lp_path) && file.exists(opt_dr_path)) {
  #rc_vec <- readRDS(opt_rc_path)
  lp_vec <- readRDS(opt_lp_path)
  dr_lst <- readRDS(opt_dr_path)
  
  ok <- which(is.finite(lp_vec))
  ## PReviously we initialized from different optimization fits - now we just pick the best!
  if (length(ok) >= 1) {
    ord <- ok[order(lp_vec[ok], decreasing = TRUE)]
    pick <- ord[[1]]
    
    # dr_lst[[pick]] should be a 1-row matrix; handle both vector/matrix robustly
    dm <- dr_lst[[pick]]
    if (!is.null(dm)) {
      row <- if (is.matrix(dm)) dm[1, , drop = TRUE] else dm
      names(row) <- colnames(dm)
      init_list <- tryCatch(flat_to_init_list(row), error = function(e) NULL)
      
      if (!is.null(init_list)) {
        init_arg <- init_list
        cat(sprintf(">>> Init from optimization: rank=%d (start=%d), lp=%g\n",
                    1, pick, lp_vec[[pick]]))
      } else {
    ...
    cat(sprintf(">>> Only %d successful optim fits; CHAIN_ID=%d too large; using init=2\n",
                length(ok), CHAIN_ID))
  }
} else {
...
if (is.list(init_arg)) {
  init_arg <- list(init_arg)
}

# ---- 6. Run NUTS (single chain) ----
fit <- mod$sample(
  data = stan_data,
  chains = 1,
  parallel_chains = 1,
...
saveRDS(fit$summary(), file.path(nutsDir, sprintf("nuts_summary_%d.Rds", CHAIN_ID)))
saveRDS(fit$draws(),   file.path(nutsDir, sprintf("nuts_draws_%d.Rds", CHAIN_ID)))

try({
  saveRDS(fit$sampler_diagnostics(), file.path(nutsDir, sprintf("nuts_diagnostics_%d.Rds", CHAIN_ID)))
})
