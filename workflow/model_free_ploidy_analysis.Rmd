...
The trajectory plot should be read as a shape check on the growth feature. Colored lines are observed live-cell trajectories by initial glucose. The dashed black line in each panel is a simple reference curve built from the median initial live-cell count for that cell line and ploidy plus the corresponding `growth_highG_median` exponential-phase rate. That rate is now defined from `G0 = 5` and `G0 = 25` wells only, using a log-linear fit from `t = 0` to the first time the well reaches 70% of its own maximum live-cell count. It is not a fit to any one well. It is there to show whether the summarized high-glucose growth rate is in the right ballpark for the observed early live-cell expansion.
...
This processing plot still shows what smoothing does to the older derivative-based growth summary. It remains useful as a QC view for trajectory shape, but the high-glucose growth signature below is no longer taken from these smoothed maxima. Instead, the high-glucose signature now uses the explicit exponential-phase fit described above.
