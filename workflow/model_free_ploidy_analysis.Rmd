...
write_model_free_agent_text_report <- function(
  ...
  effect_table <- signature_effects %>%
    left_join(feature_catalog %>% select(feature, short_label, category), by = "feature") %>%
    arrange(desc(abs(effect_per_ploidy)), category, short_label, cellLine)

  transfer_table <- transfer_feature_summary %>%
    left_join(feature_catalog %>% select(feature, short_label, category), by = "feature") %>%
  ...
  lines <- c(
    "Model-Free Ploidy Analysis Summary",
    "",
    sprintf("Generated: %s", Sys.time()),
    sprintf("Stan data: %s", paths$stan_data),
    sprintf("Feature panel source: %s", if (file.exists(paths$feature_panel)) paths$feature_panel else "rebuilt in notebook from stan_data"),
    "",
    "Feature Definitions",
    capture.output(print(as.data.frame(feature_definitions), row.names = FALSE)),
    "",
    "Per-Line Signature Values",
    capture.output(print(as.data.frame(signature_long), row.names = FALSE)),
    "",
    "Paired Ploidy Effects Per Feature",
    capture.output(print(as.data.frame(effect_table), row.names = FALSE)),
    "",
    "Transfer Ranking",
    capture.output(print(as.data.frame(transfer_table), row.names = FALSE)),
    "",
    "Transfer Case Summary",
    capture.output(print(as.data.frame(transfer_case_summary %>% arrange(desc(mean_scaled_err_improvement), desc(transfer_win_rate))), row.names = FALSE)),
    "",
    "High-G Exponential Fit QC",
    capture.output(print(as.data.frame(exp_fit_qc), row.names = FALSE)),
    ""
  )
