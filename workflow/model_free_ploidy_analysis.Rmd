...
plot_feature_scatter <- function(raw_col, title, ylab) {
  ggplot(feature_panel, aes(G0, .data[[raw_col]], color = factor(ploidy_metric))) +
    ...
    scale_x_continuous(
      ...
      breaks = c(0, 0.1, 0.25, 0.5, 1, 5, 25)
    ) +
    scale_color_brewer(palette = "Set1", name = "ploidy") +
    labs(title = title, x = "initial glucose (G0, log-like scale)", y = ylab) +
    theme_bw()
}
...
build_fit_df <- function(raw_col, max_g0 = Inf) {
  feature_panel %>%
    ...
    group_modify(~ {
      ...
      tibble(
        G0 = seq(min(.x$G0, na.rm = TRUE), max(.x$G0, na.rm = TRUE), length.out = 100),
        fitted = predict(fit, newdata = data.frame(G0 = seq(min(.x$G0, na.rm = TRUE), max(.x$G0, na.rm = TRUE), length.out = 100)))
      )
    }) %>%
    ungroup()
}
...
plot_feature_regression <- function(raw_col, title, ylab, max_g0 = Inf) {
  ...
  ggplot(feature_panel %>% filter(is.finite(.data[[raw_col]]), G0 <= max_g0), aes(G0, .data[[raw_col]], color = factor(ploidy_metric))) +
    geom_point(size = 2) +
